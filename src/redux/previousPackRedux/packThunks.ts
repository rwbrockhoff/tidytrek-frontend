...
import { PackItem, Pack } from '../../types/packTypes';
...
export const deletePack = createAsyncThunk('deletePack', async (packId: number) => {
...
export const editPackItem = createAsyncThunk(
	...
	async (packInfo: { packItemId: number; packItem: PackItem }) => {
		...
		const { data } = await tidyTrekAPI.put(`/packs/pack-items/${packItemId}`, packItem);
		return await data;
	},
);
...
export const deletePackItem = createAsyncThunk(
	...
	async (packItemId: number) => {
		const { data } = await tidyTrekAPI.delete(`/packs/pack-items/${packItemId}`);
		return await data;
	},
);
...
export const editPackCategory = createAsyncThunk(
	'editPackCategory',
	async (categoryInfo: { packCategoryId: number; packCategoryName: string }) => {
		const { packCategoryId, packCategoryName } = categoryInfo;
		const { data } = await tidyTrekAPI.put(`/packs/categories/${packCategoryId}`, {
			packCategoryName,
		});
		return await data;
	},
...
export const deletePackCategory = createAsyncThunk(
	...
	async (categoryId: number) => {
		const { data } = await tidyTrekAPI.delete(`/packs/categories/${categoryId}`);
		return await data;
	},
);
...
export const deleteCategoryAndItems = createAsyncThunk(
	...
	async (categoryId: number) => {
		const { data } = await tidyTrekAPI.delete(`/packs/categories/items/${categoryId}`);
		return await data;
	},
);
